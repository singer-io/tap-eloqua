...
BUILT_IN_BULK_OBJECTS = ["accounts", "contacts"]

QUERY_LANGUAGE_MAP = {"accounts": "Account", "contacts": "Contact"}
...
BASE_SYSTEM_FIELD = {"Id": {"type": "string"}, "CreatedAt": {"type": "string", "format": "date-time"}}

ACTIVITY_BASE_SYSTEM_FIELD = {**BASE_SYSTEM_FIELD, "Id": {"type": ["null", "string"]}}

BULK_SYSTEM_FIELDS = {**BASE_SYSTEM_FIELD, "UpdatedAt": {"type": "string", "format": "date-time"}}

CONTACT_ADDITIONAL_FIELDS = {
    "IsBounceback": {"statement": "{{Contact.Email.IsBounced}}", "type": ["null", "string"]},
    "IsSubscribed": {"statement": "{{Contact.Email.IsSubscribed}}", "type": ["null", "string"]},
    "EmailFormat": {"statement": "{{Contact.Email.Format}}", "type": ["null", "string"]},
    "AccountName": {"statement": "{{Contact.Account.Field(M_CompanyName)}}", "type": ["null", "string"]},
}
...
PKS = {"assets": ["id"], "campaigns": ["id"], "emails": ["id"], "forms": ["id"], "visitors": [], "emailGroups": ["id"]}
...
def get_type(eloqua_field):
    ...
    if internal_name == "Duration" or ((name_last_two == "Id" or name_last_two == "ID") and json_type == "number"):
        json_type = "string"
...
def to_meta(inclusion, statement, field_name):
    return {
        "metadata": {"inclusion": inclusion, "tap-eloqua.statement": statement},
        "breadcrumb": ["properties", field_name],
    }


def get_bulk_schema(
    client, stream_name, path, system_fields, query_language_name=None, activity_type=None, object_id=None
):
    params = {}
    if activity_type:
    ...
    # TODO: pagination
    data = client.get(path, params=params, endpoint="bulk_fields")
    ...
    metadata.append(
        {
            "metadata": {"tap-eloqua.id": object_id, "tap-eloqua.query-language-name": query_language_name},
            "breadcrumb": [],
        }
    )
    ...
    for prop, json_schema in system_fields.items():
        ...
        if "statement" in json_schema.keys():
            ...
            statement = "{{" + query_language_name + "." + prop + "}}"
    ...
    schema = {"properties": properties, "additionalProperties": False, "type": "object"}

    return schema, metadata


def get_bulk_obj_schema(client, stream_name, obj_name, system_fields, **kwargs):
    return get_bulk_schema(client, stream_name, "/api/bulk/2.0/{}/fields".format(obj_name), system_fields, **kwargs)
...
def get_static_schemas():
    ...
    file_names = [f for f in os.listdir(schemas_path) if os.path.isfile(os.path.join(schemas_path, f))]
    ...
    for file_name in file_names:
        ...
        for prop, json_schema in schema["properties"].items():
            if prop in pk:
                ...
                inclusion = "available"
            metadata.append({"metadata": {"inclusion": inclusion}, "breadcrumb": ["properties", prop]})
...
def get_schemas(client):
    ...
    for bulk_object in BUILT_IN_BULK_OBJECTS:
        ...
        json_schema, metadata = get_bulk_obj_schema(client, bulk_object, bulk_object, system_fields)
        SCHEMAS[bulk_object] = json_schema
        FIELD_METADATA[bulk_object] = metadata

    for activity_type in ACTIVITY_TYPES:
        stream_name = activity_type_to_stream(activity_type)
        json_schema, metadata = get_bulk_obj_schema(
            client, stream_name, "activities", ACTIVITY_BASE_SYSTEM_FIELD, activity_type=activity_type
        )
    ...
    for custom_obj in data["items"]:
        groups = re.match(r"/customObjects/([0-9]+)", custom_obj["uri"]).groups()
        object_id = groups[0]

        # TODO: more normalization?
        stream_name = custom_obj["name"].strip().lower().replace(" ", "_").replace("-", "_")
