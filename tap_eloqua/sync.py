...
from singer import UNIX_SECONDS_INTEGER_DATETIME_PARSING, Transformer, metadata, metrics
...
def write_bulk_bookmark(state, stream_name, sync_id, offset, max_updated_at):
    if "bookmarks" not in state:
        state["bookmarks"] = {}
    state["bookmarks"][stream_name] = {"sync_id": sync_id, "offset": offset, "datetime": max_updated_at}
    singer.write_state(state)
...
def persist_records(catalog, stream_id, records, activity_type=None):
    ...
    with metrics.record_counter(stream_id) as counter:
        for record in records:
            ...
            with Transformer(integer_datetime_fmt=UNIX_SECONDS_INTEGER_DATETIME_PARSING) as transformer:
...
def stream_export(
    ...
    while has_more:
        ...
        data = client.get(
            ...
            params={"limit": bulk_page_size, "offset": offset},
...
class ActivityExportTooLarge(Exception):
    """An Eloqua Bulk Activity export can have a maximum of 5 million rows in
    it.

    Because rows in these exports are not ordered, an export that has 5
    million records in it is an error, and the date window must be cut
    in half until it fits into a single export.
    """
...
def sync_bulk_obj(client, catalog, state, start_date, stream_name, bulk_page_size, activity_type=None, end_date=None):
    ...
    for meta in stream.metadata:
        ...
        elif meta["metadata"].get("selected", True) or meta["metadata"].get("inclusion", "available") == "automatic":
    ...
    with metrics.job_timer("bulk_export"):
        log_string = "{} - Creating bulk export from {}".format(stream_name, last_date)
        if end_date:
            log_string += " to {}".format(end_date.to_datetime_string())
        LOGGER.info(log_string)

        data = client.post("/api/bulk/2.0/{}/exports".format(url_obj), json=params, endpoint="export_create_def")

        data = client.post(
            "/api/bulk/2.0/syncs", json={"syncedInstanceUri": data["uri"]}, endpoint="export_create_sync"
        )
        ...
        while True:
            data = client.get("/api/bulk/2.0/syncs/{}".format(sync_id), endpoint="export_sync_poll")

            status = data["status"]
            if status == "success":
                break
            elif status not in ["pending", "active"]:
                message = "{} - status: {}, exporting failed".format(stream_name, status)
                LOGGER.error(message)
                raise Exception(message)
            elif (time.time() - start_time) > MAX_RETRY_ELAPSED_TIME:
                message = "{} - export deadline exceeded ({} secs)".format(stream_name, MAX_RETRY_ELAPSED_TIME)
                LOGGER.error(message)
                raise Exception(message)
    ...
    # Check record count
    data = client.get("/api/bulk/2.0/syncs/{}/logs".format(sync_id), endpoint="export_sync_poll")
...
def sync_static_endpoint(client, catalog, state, start_date, stream_id, path, updated_at_col):
    ...
    while True:
        ...
        if records:
            max_updated_at = pendulum.from_timestamp(int(records[-1][updated_at_col])).to_iso8601_string()
...
def sync_activity_stream(client, stream_name, state, catalog, start_date, bulk_page_size, activity_type):
    finished = False
...
def sync(client, catalog, state, start_date, bulk_page_size):
    ...
    for bulk_object in BUILT_IN_BULK_OBJECTS:
        should_stream, last_stream = should_sync_stream(selected_streams, last_stream, bulk_object)
        if should_stream:
            update_current_stream(state, bulk_object)
            sync_bulk_obj(client, catalog, state, start_date, bulk_object, bulk_page_size)
    ...
    for activity_type in ACTIVITY_TYPES:
        ...
        if should_stream:
            sync_activity_stream(client, stream_name, state, catalog, start_date, bulk_page_size, activity_type)

    for stream_name in get_custom_obj_streams(catalog):
        should_stream, last_stream = should_sync_stream(selected_streams, last_stream, stream_name)
        if should_stream:
            update_current_stream(state, stream_name)
            sync_bulk_obj(client, catalog, state, start_date, stream_name, bulk_page_size)

    static_endpoints = [
        {"stream_id": "visitors", "path": "data/visitors", "updated_at_col": "V_LastVisitDateAndTime"},
        {"stream_id": "campaigns", "path": "assets/campaigns", "updated_at_col": "updatedAt"},
        {"stream_id": "emails", "path": "assets/emails", "updated_at_col": "updatedAt"},
        {"stream_id": "forms", "path": "assets/forms", "updated_at_col": "updatedAt"},
        {"stream_id": "assets", "path": "assets/externals", "updated_at_col": "updatedAt"},
        {"stream_id": "emailGroups", "path": "assets/email/groups", "updated_at_col": "updatedAt"},
    ]

    for static_endpoint in static_endpoints:
        stream_id = static_endpoint["stream_id"]
        should_stream, last_stream = should_sync_stream(selected_streams, last_stream, stream_id)
        if should_stream:
            update_current_stream(state, stream_id)
            path = static_endpoint["path"]
            updated_at_col = static_endpoint["updated_at_col"]
            sync_static_endpoint(client, catalog, state, start_date, stream_id, path, updated_at_col)
